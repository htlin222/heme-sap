...
export function renderPlaylist(items, { doneSet, currentIndex, query, onlyTodo }) {
  ...
  $("#playlist").innerHTML =
    html.join("") ||
    `<div class="Blankslate">${icon("inbox", 28)}<p class="Blankslate__heading">No matching videos</p></div>`;
  $("#playlistCount").textContent =
    shown === items.length ? `${items.length} videos` : `${shown} / ${items.length}`;
  return shown;
}
  ...
  $("#playerInfo").innerHTML = `
    <div class="Player__bar">
      ...
      <div class="Player__actions">
        ...
        <button class="btn btn-icon" data-toggle-list type="button" title="Hide / show list">${icon("layers", 16)}<span class="visually-hidden" data-list-label>Hide list</span></button>
        ${discussButton()}
    ...
    ${
      item.why || item.assessment
        ? `<details class="Player__more">
             ...
             ${item.assessment ? `<p class="Player__note"><strong>Self-check </strong>${esc(item.assessment)}</p>` : ""}
           </details>`
        : ""
    }
    ${discussPanel()}`;
