...
const SHEET = [
  ["Playback", [["Space / K", "Play / pause"], ["J", "Back 10s"], ["L", "Forward 10s"], ["← / →", "±5s"], ["0–9", "Jump to 0%–90%"]]],
  ["Sound & picture", [["M", "Mute"], ["↑ / ↓", "Volume ±10"], ["F", "Fullscreen"], ["Shift + . / ,", "Speed up / down"]]],
  ["Course navigation", [["Shift + N", "Next video"], ["Shift + P", "Previous video"], ["/", "Search"], ["?", "Show this sheet"]]],
];
...
function toggleSheet(force) {
  ...
  if (!el) {
    ...
    el.innerHTML = `
      <div class="KeySheet__box" role="dialog" aria-label="Keyboard shortcuts">
        <div class="KeySheet__head">
          ${icon("info", 16)}<strong>Keyboard shortcuts</strong>
        ...
        <p class="KeySheet__foot">Playback commands are sent through the YouTube player API; once you click into the video, YouTube's own shortcuts work too.</p>
      </div>`;
...
export function bindKeys({ next, prev, isPlayerTab }) {
  addEventListener("keydown", (e) => {
    ...
    } else if (k === "j" || k === "J") { e.preventDefault(); seek(-10); toast("−10s"); }
    else if (k === "l" || k === "L") { e.preventDefault(); seek(10); toast("+10s"); }
    else if (k === "ArrowLeft") { e.preventDefault(); seek(-5); toast("−5s"); }
    else if (k === "ArrowRight") { e.preventDefault(); seek(5); toast("+5s"); }
    else if (k === "m" || k === "M") {
      e.preventDefault();
