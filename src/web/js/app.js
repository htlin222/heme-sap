...
/** 類型篩選鈕依設定檔的 kinds 產生，框架本身不認識任何主題的類型名稱 */
function renderKindFilters(c) {
  const bar = $("#kindFilters");
  if (!bar) return;
  const all = bar.querySelector('[data-filter="all"]');
  bar.innerHTML = "";
  if (all) bar.append(all);
  for (const k of c.kinds || []) {
    const btn = document.createElement("button");
    btn.className = "FilterBar__btn";
    btn.type = "button";
    btn.dataset.filter = k.id;
    btn.innerHTML =
...
function applyChrome(data) {
  ...
  $("#search")?.setAttribute("placeholder", c.ui?.searchPlaceholder || "Search…");
  ...
  set(
    ".Hero__lede",
    (c.hero?.lede || "")
      // {units} 指課程的主要單元數（本課＝書的節數），不是含補充影片的欄位總數
      .replace("{units}", data.meta.lesson_units)
      .replace("{videos}", data.meta.video_unique)
...
function renderStats() {
  ...
  // 單元數／影片欄位數／去重支數是三個不同的東西，講清楚免得讀者對不上
  $("#heroNote").innerHTML =
    `${meta.units} entries = ${meta.lesson_units} core lectures + ${meta.drill_units} supplementary videos` +
    (meta.alt_lessons
      ? `, plus ${meta.alt_lessons} alternate-language versions, bringing the playlist to ${meta.video_slots} slots`
      : "") +
    `. After removing videos shared across sections, that is ${meta.video_unique} distinct videos, ` +
    `${meta.duration} of runtime.`;
}
...
function updateChapterMeta() {
  state.course.chapters.forEach((ch) => {
    ...
    $(".Chapter__meta", el).textContent =
      `${ch.units.length} units${drillTotal ? ` · ${drillTotal} videos` : ""}${done ? ` · ${done} done` : ""}`;
  });
}
...
function playAt(i) {
  ...
  if (load(STORE.wide, false)) {
    ...
    $("[data-list-label]").textContent = "Show list";
  }
  refreshPlaylist();
...
function bindEvents() {
  ...
  $("#playerInfo").addEventListener("click", (e) => {
    ...
    if (wide) {
      ...
      save(STORE.wide, on);
      $("[data-list-label]").textContent = on ? "Show list" : "Hide list";
      requestAnimationFrame(fitFrame);
      return;
    }
  ...
  $("#resetProgress").addEventListener("click", () => {
    ...
    if (!confirm(`Clear the completion record for ${state.done.size} units?`)) return;
    state.done.clear();
    save(STORE.done, []);
    $$(".Unit").forEach((u) => {
...
async function init() {
  ...
  try {
    ...
    $("#chapters").innerHTML = `
      <div class="Blankslate">
        ...
        <p class="Blankslate__heading">Could not load the course data</p>
        <p>${esc(err.message)}</p>
      </div>`;
    return;
  }
  ...
  $("#tabCourseCount").textContent = data.meta.lesson_units;
