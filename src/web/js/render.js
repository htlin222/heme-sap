    ...
    return `
      <div class="VideoCard VideoCard--missing">
        ...
        <span class="VideoCard__main">
          <span class="VideoCard__title">No qualifying video found yet</span>
          <span class="VideoCard__meta">${esc(v?.note || "Nothing on YouTube meets this course's quality bar for this topic")}</span>
        </span>
        ${playBtn(false, true)}
      </div>`;
  }
...
const langLabel = (l) => (CFG.languages || {})[l] || l || "Other";
...
function lessonBox(u) {
  ...
  return `
    <div class="LessonBox">
      <div class="LessonBox__langs" role="tablist" aria-label="Lesson language">
        ${lessons
          .map(
            (l, i) => `
    ...
    .map(
      (m) =>
        `<button class="Label Label--neutral Label--muscle" data-muscle="${esc(m)}"
                 type="button" title="Filter to content involving ${esc(m)}">${esc(m)}</button>`,
    )
    .join("");
}
...
function drill(d) {
  ...
  // 有連結就整列可點，跟主課卡片一致
  return d.url
    ? `<li ${attrs}><a class="Drill__link" href="${esc(d.url)}" target="_blank" rel="noopener" title="${esc(d.title || "Watch")}">${inner}</a></li>`
    : `<li ${attrs}><span class="Drill__link" aria-disabled="true" title="No qualifying video found yet">${inner}</span></li>`;
}
  ...
  return `
    <section class="Evidence" data-evidence="${esc(unitId)}">
      ...
      <div class="Evidence__body">
        ...
        ${
          ev.url
            ? `<div class="Evidence__cite"><a href="${esc(ev.url)}" target="_blank" rel="noopener">Read the full answer on OpenEvidence ${icon("external-link", 11)}</a></div>`
            : ""
        }
      </div>
    </section>`;
  ...
  return `
    <section class="Evidence DrillEv" data-drillev="${esc(u.id)}">
      <button class="Evidence__header" type="button" data-toggle="drillev">
        ...
        <span class="Counter">${cats.length} topics · ${totalCites} papers</span>
        <span class="Evidence__spacer"></span>
  ...
  const badges = [
    ...
    u.evidence?.evidence_grade
      ? `<span class="Label ${toneCls(gradeOf(u.evidence.evidence_grade))}"
               title="OpenEvidence verification result">${icon("microscope", 11)} ${gradeOf(u.evidence.evidence_grade).label}</span>`
      : "",
  ].join("");
    ...
    <article class="Unit${done ? " is-done" : ""}" id="${esc(u.id)}" data-unit="${esc(u.id)}"
      ...
      <button class="Unit__header" type="button" data-toggle="unit">
        <span class="Unit__check" data-action="toggle-done" role="checkbox"
              aria-checked="${done}" tabindex="0" title="Mark as done">${icon("check", 12)}</span>
...
export function renderStance(stance) {
  ...
  const card = (s, i) => {
    ...
    const findings = (s.key_findings || []).length
      ? `<details>
           <summary>See the full evidence (${s.key_findings.length} findings)</summary>
          ...
          ${
            s.url
              ? `<div class="StanceCard__cites" style="margin-top:6px">
                   <a href="${esc(s.url)}" target="_blank" rel="noopener">Read the full answer on OpenEvidence ${icon("external-link", 10)}</a>
                 </div>`
              : ""
          }
        </footer>
      ...
      <button class="Chapter__header" type="button" data-toggle="chapter">
        ...
        <span class="Chapter__titles">
          ...
          <span class="Chapter__meta">
            ${ch.units.length} units${drillTotal ? ` · ${drillTotal} videos` : ""}
    ...
    return `
      <button class="ChapterCard" type="button" data-goto-chapter="${esc(ch.code)}">
        ...
        <span class="ChapterCard__main">
          ...
          <span class="ChapterCard__meta">${ch.units.length} units${drills ? ` · ${drills} videos` : ""}</span>
...
${esc(L.stanceLede || "")}
      ...
      <button class="btn" type="button" data-tab-link="stance">
        ${esc(CFG.ui?.tabs?.stance || "Positions")} ${icon("chevron-right", 14)}
      </button>
    </section>
