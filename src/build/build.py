...
def load_json(path: Path):
    ...
    try:
        ...
        print(f"  ✗ {path.name} failed to parse as JSON: {e}", file=sys.stderr)
        return None
...
def main() -> int:
    ...
    for code, title, ic, src in CHAPTERS:
        ...
        if not blob:
            problems.append(f"{code}: source file data/{src}.json not found")
            continue
        ...
        if not node:
            problems.append(f"{code}: chapter not found in source file data/{src}.json")
            continue
        ...
        if len(units) != want_units:
            problems.append(f"{code}: {len(units)} units, expected {want_units}")
        if got_drills != want_drills:
            problems.append(f"{code}: {got_drills} supplementary videos, expected {want_drills}")
        ...
        for u in units:
            ...
            for v in vids:
                ...
                if not url:
                    missing_urls.append(
                        f"{u['id']} / {v.get('name') or v.get('title') or 'core lesson video'}"
                    )
                    continue
                if not YT.match(url):
    ...
    muscle_index = [
        {
            "name": m,
            "group": facets.GROUP_OF.get(m, "Other") if facets else "Other",
    ...
    group_order = [*(facets.GROUPS if facets else []), "Other"]
    ...
    course = {
        ...
        "meta": {
            ...
            # concept-* 是首頁的立場卡，不是課程單元，不計入「有實證的單元數」
            "evidence_checked": sum(1 for k in evidence if not k.startswith("concept-")),
        },
        "chapters": chapters,
    }
    ...
    print(f"   Core lessons {unit_total} · supplementary videos {drill_total} · total {unit_total + drill_total}")
    print("   " + " / ".join(f"{k['label']} {kinds[k['id']]}" for k in CFG.get("kinds", [])))
    print(
        f"   Videos: {unit_total + drill_total + alt_count[0]} slots"
        f" (incl. {alt_count[0]} alternate-language versions) · {len(seen_urls)} after dedupe"
    )
    print(
        f"   Runtime {fmt_duration(seconds)}"
        f" · all language versions {fmt_duration(seconds_all[0])}"
        f" · evidence checked {course['meta']['evidence_checked']} sections"
    )
    print(
        f"   YouTube metadata {meta_hits[0]}/{meta_hits[0] + len(meta_miss)} hits"
        f" · facet index {len(muscle_index)} entries"
        f" · categories {len(cat_counts)} (with references: {len(drill_ev)})"
    )
    # 沒有 category 模組時「未分類」是定義上的必然，不必當警告吵
    if uncategorised and categories:
        print(
            f"   ⚠ Uncategorised items {len(uncategorised)}: {', '.join(x for x in uncategorised[:5] if x)}"
        )

    if missing_urls:
        print(f"\n⚠ Missing video links: {len(missing_urls)}")
        for m in missing_urls[:15]:
            print(f"   · {m}")
        if len(missing_urls) > 15:
            print(f"   … and {len(missing_urls) - 15} more")
    if bad_urls:
    ...
    if same_unit_dupes:
        print(f"\n✗ Videos repeated inside a single unit: {len(same_unit_dupes)}")
        for (uid, url), n in same_unit_dupes.items():
            print(f"   · {uid} uses {url} {n} times")
        problems.append(f"duplicate videos within a unit: {len(same_unit_dupes)}")
    if dupes:
        print(f"\n· Videos shared across units: {len(dupes)} (a small overlap is allowed)")
    if problems:
        print(f"\n✗ Quota mismatches: {len(problems)}")
        for p in problems:
            print(f"   · {p}")
        return 1
