...
def build_schema(course: dict) -> dict:
    ...
    # What the course says it teaches. Capped — a syllabus of several hundred
    # entries is noise in structured data, not signal.
    topic_type = (CFG.get("ui") or {}).get("problemType") or "posture"
    teaches = [u["name"] for ch in chapters for u in ch["units"] if u.get("type") == topic_type]
    teaches = teaches[:60]
    ...
    syllabus = [
        {
            ...
            "description": (
                ...
                + (
                    f", with {sum(len(u.get('drills') or []) for u in ch['units'])}"
    ...
    return {
        ...
        "@graph": [
            ...
            },
            {
                ...
                "educationalLevel": SITE_CFG.get("educationalLevel", "Beginner"),
...
def render_template(meta: dict) -> None:
    ...
    def sub(m):
        ...
        return (
            str(val)
            .replace("{units}", str(meta.get("lesson_units", meta["units"])))
            .replace("{videos}", str(meta.get("video_unique", 0)))
...
def write_robots() -> None:
    (PUB / "robots.txt").write_text(
        ...
        "Allow: /\n"
        "\n"
        "# AI retrievers are welcome — the cited evidence here is meant to be quoted\n"
        "User-agent: GPTBot\n"
        "Allow: /\n"
        "\n"
...
def write_llms(course: dict) -> None:
    ...
    lines = [
        ...
        + f" {meta['lesson_units']} sections, {meta['drill_units']} supplementary videos,"
        f" {meta['video_unique']} distinct videos, {meta['duration']} of runtime.",
        "",
        f"## {CFG.get('stance', {}).get('title', 'Position')} (important)",
        "",
